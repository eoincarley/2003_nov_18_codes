...
pro plot_rstn_rhessi, t0, t1, position=position
	...
	outplot, times_rate, data_0, color=10, thick=4
	outplot, times_rate, data_1, color=8, thick=5
	outplot, times_rate, data_2, color=4, thick=5
...
pro plot_rstn_rhessi_flux, postscript=postscript
	...
	result = read_ascii(file, data_start=6393)
	data = result.field1
	dshape = size(data)
	nseconds = dshape[2]
	nfreqs = dshape[1]
	...
	flux_245 = data[1, tindices] 
	flux_410 = data[2, tindices] 
	flux_610 = data[3, tindices] 
	flux_1415 = data[4, tindices] 
	flux_2695 = data[5, tindices] 
	flux_4995 = data[6, tindices] 
	flux_8800 = data[7, tindices] 
	flux_15499 = data[8, tindices] 
	...
	if keyword_set(postscript) then begin	
		setup_ps, output_path+'rstn_rhessi_flux_20031118.eps'
	endif else begin
		loadct, 0
		!p.background=100
		!p.color=0
	...
	plt_fmt, times, flux_245, pos0, colors[0], freqs[0], xrange, xtickfmt='(A1)', xtitle=' ', yr=[1e1, 1.7e3], tag='a'
	plt_fmt, times, flux_410, pos1, colors[1], freqs[1], xrange, xtickfmt='(A1)', xtitle=' ', yr=[1e1, 1.5e4], tag='b'
	plt_fmt, times, flux_610, pos2, colors[2], freqs[2], xrange, xtickfmt='(A1)', xtitle=' ', yr=[1e1, 1.5e4], tag='c'
	plt_fmt, times, flux_1415, pos3, colors[3], freqs[3], xrange, xtickfmt='(A1)', xtitle=' ', yr=[1e1, 1.5e4], tag='d'
	plt_fmt, times, flux_2695, pos4, colors[4], freqs[4], xrange, xtickfmt='(A1)', xtitle=' ', yr=[1e2, 2.5e3], tag='e'
	plt_fmt, times, flux_4995, pos5, colors[5], freqs[5], xrange, xtickfmt='(A1)', xtitle=' ', yr=[1e2, 2.5e3], tag='f'
	plt_fmt, times, flux_15499, pos6, colors[6], freqs[7], xrange, xtickfmt='', xtitle='Time (UT)', yr=[1e2, 2.5e3], tag='g'


	t0plot=anytim('2003-11-18T08:05:00', /utim)
	t1plot=anytim('2003-11-18T08:35:00', /utim)
	plot_rstn_rhessi, t0plot, t1plot, position=pos7
